...
\section{Performance evaluation}\label{sec:performance}
	...
	For the messages sent to the the server, the minimum size is the \textit{148B}, the rest depends on the number of contacts the node had.
	
	For a simulation with twenty nodes, each of them sends \hl{X} messages in broadcast a second.
	In the ``\textit{worst case scenario}'' where all the nodes are in infecting range, the lenght of the message sent to the server would be \textit{2.41KB} (\textit{2.49KB} with the payloads).
...
\section{Future work}\label{sec:future_work}
	...
	\subsection{Nodes}
		...
		\hl{Nodes logic can be improved by }
	...
	\subsection{DB consumer}
		...
		As per the broker server, scaling the DB consumer node and making each node read from a separate queu or a separate server would improve network performance and fault tolerance.
